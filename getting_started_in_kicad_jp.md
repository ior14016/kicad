...
電化製品などの中に必ず存在するのが`プリント基板(PCB)`です．<br>
プリント基板は小型・軽量・大量生産に向いており，スマートフォンなどの電子機器が一般的となった現代社会には欠かせないものとなっています．<br>
そんなプリント基板を設計できるソフトウェアが`KiCAD`です．<br>
...
実際に製品となる プリント基板を`PCBエディター`を使って製作していきます．<br>
...
このセクションは プリント基板を製作する際，完成度合いを大きく左右する重要なセクションです．<br>
...
次に，`ファイル` -> `基板の設定` で プリント基板の製造方法を定義します．<br>
...
新しいリストが追加されるので，`名前`に`PWR`と入力し，`配線幅`を`0.6mm`に変更します．<br>
...
初期設定が終わりました．<br>早速，プリント基板上に部品を乗せていきましょう．<br>
...
この状態で プリント基板の 3D 表示を行うと，配線が追加されているのがわかります．<br>

## 基板のアウトライン(外形線)，レイヤーの選択

先程から プリント基板の 3D 表示を行うと，`基板のアウトラインが欠如している`とのエラーが表示されています．<br>
...
実際，プリント基板の 3D 表示を行うと，`基板のアウトラインが欠如している`とのエラーが引き続き表示されています．<br>
...
この状態で先のように，回路を囲むように 2 点を選択すると，無事に基板のアウトラインが描画され，プリント基板の 3D 表示を行うと，エラーが表示されなくなっていると思います．<br>

`矩形を描画`以外にも，`線を描画`，`円弧を描画`，`円を描画`，`ポリゴンを描く`などがあります．<br>
...
プリント基板の 3D 表示を行なって，確認してみましょう．<br>
...
工房に設置されている プリント基板加工機用の加工データの製作方法<br>
